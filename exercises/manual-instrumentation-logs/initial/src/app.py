...
def get_user():
    ...
    if user is not None:
        ...
        logging.warning("Could not find user with id 123")
...
@app.route("/")
def index():
    logging.warn("Info from the index function")
    do_stuff()
...
## OpenTelemetry integrates itself into existing logging modules via its Logging Bridge API
## You can enable OpenTelemetry logging by automatic instrumention or you set up logging manually